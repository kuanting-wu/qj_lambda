...
// Handle YouTube Channel Videos
const handleYouTubeChannelVideos = async (event, db, user) => {
    const { getValidAccessToken, getChannelVideos } = require('./youtube-api');
    ...
    try {
        ...
        try {
            ...
            const accessToken = await getValidAccessToken(db, user.user_id);
            
            // Get videos from YouTube API
            const videosResponse = await getChannelVideos(accessToken, pageToken);
            
            return {
                statusCode: 200,
                body: JSON.stringify(videosResponse)
            };
        } catch (apiError) {
            console.error('Error calling YouTube API:', apiError);
            
            // If user hasn't authenticated with YouTube, return specific error
            if (apiError.message === 'No YouTube credentials found for this user') {
                return {
                    statusCode: 403,
                    body: JSON.stringify({ 
                        error: 'YouTube authentication required', 
                        needsAuth: true,
                        authUrl: require('./youtube-api').getYouTubeAuthUrl()
                    })
                };
            }
            
            // If in development mode, use mock data for testing
            if (process.env.NODE_ENV === 'development') {
                console.log('Using mock data for development');
                // Generate mock video data
                const mockVideos = Array(10).fill().map((_, i) => {
...
const handleYouTubeAuthCallback = async (event, db, user) => {
    const { exchangeCodeForTokens, saveYouTubeCredentials } = require('./youtube-api');
    ...
    try {
        ...
        await saveYouTubeCredentials(db, user.user_id, tokenData);
        
        return {
            statusCode: 200,
            body: JSON.stringify({ success: true, message: 'YouTube account connected successfully' })
        };
    } catch (error) {
        console.error('Error handling YouTube auth callback:', error);
        return {
            statusCode: 500,
            body: JSON.stringify({ error: 'Failed to connect YouTube account', message: error.message })
        };
    }
};
